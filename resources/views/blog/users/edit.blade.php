...
<!-- card body -->
  ...
  <h4 class="text-3xl text-gray-700 mb-5 flex items-center">Edit Data Users</h4>
  <div class="p-10 rounded-md shadow-md bg-white">
    <form action="{{ route('users.update', (string) $data->_id ) }}" method="post" enctype="multipart/form-data">
      @csrf
      @method('PUT')
   <div class="mb-6">
    <label class="block mb-3 text-gray-600" for="">Name</label>
    <input type="text" name="name"class="border border-gray-500 rounded-md inline-block py-2 px-3 w-full text-gray-600 tracking-wider" value="{{old('name',$data->name)}}"/>
    @error('name')
   ...
   <div class="mb-6">
    ...
    <input
     type="text" name="username" class="border border-gray-500 rounded-md inline-block py-2 px-3 w-full text-gray-600 tracking-widest" value="{{old('username',$data->username)}}"/>
     @error('username')
      <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
     @enderror
   </div>
   ...
   <div class="mb-6">
    ...
    <input
     type="text" name="email" class="border border-gray-500 rounded-md inline-block py-2 px-3 w-full text-gray-600 tracking-widest" value="{{old('email',$data->email)}}"/>
     @error('email')
      <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
     @enderror
   </div>
 ...
 <div class="">
    ...
    <input
     type="file" name="pic"class="border border-gray-500 rounded-md sm:inline-block py-2 px-3 w-full text-gray-600 tracking-widest" value="{{old('pic',$data->pic)}}" autocomplete="off"/>
      @error('pic')
      <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
     @enderror
   </div>
    ...
    <input
     type="text" name="password" class="border border-gray-500 rounded-md sm:inline-block py-2 px-3 w-full text-gray-600 tracking-widest" autocomplete="off"/>
     @error('password')
      <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
     @enderror
   </div>
